...
const MangasPage = () => {
  ...
  const { items, loading, error } = useMangaLibrary();
  const [searchQuery, setSearchQuery] = useState("");
  const [statusFilter, setStatusFilter] = useState<MangaReadingStatus | "all">("all");

  const filteredItems = useMemo(() => {
    return items.filter((item) => {
      const matchesSearch = item.title.toLowerCase().includes(searchQuery.toLowerCase());
      const matchesStatus = statusFilter === "all" || item.readingStatus === statusFilter;
  ...
  // Handle case where API is not yet ready (404/Error) or truly empty
    ...
    <div className="space-y-6">
      ...
      {hasNoData ? (
        <EmptyState 
          ...
          title={error ? "Bibliothèque indisponible" : "Votre bibliothèque est vide"}
          description={
            error 
        ...
        <div className="space-y-4">
          {/* Bar de recherche locale et filtres */}
          <div className="flex flex-col gap-4 sm:flex-row sm:items-center">
            ...
            <div className="flex items-center gap-2 overflow-x-auto pb-2 sm:pb-0">
              ...
              <select
                value={statusFilter}
                onChange={(e) => setStatusFilter(e.target.value as MangaReadingStatus | "all")}
